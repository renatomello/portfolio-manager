...
class Investments():
    ...
    def hyperparameters(self):
        ...
        self.domestic_database = self.kwargs.get('domestic_database', 'brazil_stocks')
    ...
    def get_benchmarks(self):
        ...
        self.bova['close_dollar'] = [float(x)*float(y) for x, y in zip(self.bova.close.to_list(), self.dollar_full.close.to_list())]
    ...
    def get_quotas(self, asset):
        ...
        for asset in list(quotas.keys()):
            if asset in self.fractions:
            ...
            elif asset in self.domestic_tickers:
                ...
                close_price = read_sql_query("SELECT close FROM {} WHERE ticker = '{}' ORDER BY date DESC LIMIT 1".format(self.domestic_options_database, asset), connection).values.flatten()[0]
            ...
            if domestic == True:
                value_usd.append(float(close_price) * float(quotas.get(asset)) * float(self.dollar))
                value_brl.append(float(close_price) * float(quotas.get(asset)))
        ...
        for quote in quotes:
            if quote in self.interests:
                ...
                if quote in self.fractions:
                    ...
                    prices = prices.loc[prices.date >= '2019-01-01'].dropna() if quote == 'CLOV' else prices
                    prices = self.insert_weekends(prices)
                elif quote in self.domestic_options_tickers:
                    prices = read_sql_query("SELECT date, close FROM {} WHERE ticker = '{}' ORDER BY date".format(self.domestic_options_database, quote), connection).drop_duplicates('date')
                    prices = self.insert_weekends(prices)
                else:
                    prices = read_sql_query("SELECT date, close FROM {} WHERE ticker = '{}' ORDER BY date".format(self.domestic_database, quote), connection).drop_duplicates('date')
                    prices = self.insert_weekends(prices)
                    lista = list()
                    for date, price in zip(prices.date, prices.close):
                        date = self.dollar_full.date.iloc[0] if date < self.dollar_full.date.iloc[0] else date
                        conversion = self.dollar_full.loc[self.dollar_full.date ==  date, 'close']
                        lista.append(float(price) * float(conversion))
                    prices['close'] = lista
                connection.close()
                engine.dispose()
                for data, share in zip(df.loc[quote].date, df.loc[quote].share):
                    close_price = prices.loc[prices.date >=  data]
                    close_price['portfolio'] = [float(price) * float(share) for price in close_price.close]
